...
begin
	# Example:
	eng_L = wing_coo[1,2] - [-1.25843, 0., -1.] # Left engine, at mid-section leading edge
	eng_R = wing_coo[1,4] - [-1.25843, 0., -1.] # Right engine, at mid-section leading edge
end
...
htail = WingSection(
    ...
    sweep       = 25.9, #2.02,  			# Sweep angle (deg)
    ...
    position    = fuse_end - [ 1.656, 0., -2.232], # HOW DO YOU DETERMINE THIS?
)
...
# ╔═╡ bfab1f32-0fe6-4477-8c54-96c93b4246e0
...
vtail = WingSection(
    area        = 6.3, 			# Area (m²). # HOW DO YOU DETERMINE THIS?
    aspect      = 1.12,  			# Aspect ratio
    taper       = 0.66, #0.59831,  			# Taper ratio
    sweep       = 52.77, 			# Sweep angle (deg)
...
begin
	...
	TOGW 	= 7066.64 * g # Takeoff gross weight, N
	W_other = 0.17 * TOGW # All other components

	# Engine
	W_engine 	 = 298 * g # FJ44-4A engine weight (single), N
