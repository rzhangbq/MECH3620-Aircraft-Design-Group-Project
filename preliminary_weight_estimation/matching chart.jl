...
# ╔═╡ b49f582e-fd93-4251-aec1-7a7515e79a4b
### A Pluto.jl notebook ###
# v0.19.38
begin
	using Markdown
...
# ╔═╡ 39dd310b-d52d-4b3f-b30c-74cde85ec2f1
begin
	""" These are variables """
	CD = 0
	L = 0
...
# ╔═╡ 69df8d21-9b5c-4117-9e34-0990b715dca1
begin
...
# ╔═╡ 0f81f778-85c9-45b3-9b7d-1a1bd9aaf6ec
Wcrew

# ╔═╡ 04f3e59a-37e6-4a70-a18e-bb300f3a1d7c
cruise1FF = cruise_weight_fraction(R1, cruise_SFC, Cruise_speed, LD_cruise)


# ╔═╡ 6b9843f5-7d8f-4fb9-bef4-47d73d2b6d34
Wf_man = 1.06 * (1 - takeoffFF * climbFF * cruise1FF * landingFF)

# ╔═╡ c3fafd94-ea9f-4528-8920-b31fcdc5c601
WTOs, erros = compute_maximum_takeoff_weight(7500g, Wpayload, Wcrew, Wf_man, A, B)

# ╔═╡ dd791f09-eeb7-4f98-8ab9-10ea6a76423d
# cruise1FF = 0.85

# ╔═╡ 102dff6f-8a67-4582-9907-6aefad561d5f
# matching chart takeoff
begin
...
# ╔═╡ 217b7bf5-fa7d-40c6-bf36-26f1489bdd05
takeoff_condition(WbS, σ, CL_takeoff, TOP) = (0.0929/4.448) * WbS / (σ * CL_takeoff * TOP)
# unit convert!

# ╔═╡ 2d443f33-682b-4d5a-9169-fbb98582c3e8
wing_loadings = 200:12000; # W / S_ref

# ╔═╡ 2d4e942b-9d70-48c7-b004-0104d2148a56
TbW_takeoff = takeoff_condition.(wing_loadings, σ, CL_takeoff, TOP)

# ╔═╡ 58d0f732-e84b-48c9-996b-a645197042f3
begin
...
# ╔═╡ 61209335-6a56-4224-9d24-444e7a2ecf68
...
begin
	...
	r = 0.6			# safety margin of FAR 25 
	n = 15
end

# ╔═╡ f4252091-03ab-4611-b403-e5d89348dc7f
landing_condition(s_L, s_a, σ, CL_max, g = 9.81) = ((s_L - s_a) * (g * σ * CL_max)) / 5 # landing_condition = W_L / S_ref

# ╔═╡ d1770adb-8612-4946-9cda-ad7a7bf9f33f
# convert back to the takeoff sea-level-statistic (sls) condition
landing_sls = landing_condition(s_L, s_a, σ, CL_landing) / landingFF

# ╔═╡ 03fb195a-e37a-4ade-ba32-8a95bf86a3e0
wbs_landing = fill(landing_sls, n);

# ╔═╡ 08538f58-ca4f-4aa6-94ef-7098711f2f39
TbWs = range(0, 0.5, length = n);

# ╔═╡ b0638c98-f73b-4a31-bb2d-6940b8dcac70
begin
	plot(ylabel = "(T/W)", xlabel = "(W/S), N/m²", title = "Matching Chart")
	# plot!(stalls, TbWs, label = "Stall")
	plot!(wing_loadings, TbW_takeoff, label = "Takeoff")
...
# ╔═╡ 0ff9408d-14ba-4773-aa70-0e89065d3f9c
# stall 
begin 
...
# ╔═╡ f9e9b981-d9c8-4ecc-ac47-a3d718946cb6
dynamic_pressure(rho, V) = 1/2 * rho * V^2

# ╔═╡ 7a7bdb1f-f49d-43f9-906d-317d7256af89
wind_loading_stall(V_stall, CL_max, rho) = dynamic_pressure(rho, V_stall) * CL_max

# ╔═╡ fe9106b1-1c3d-4506-bc34-c6402ff9177b
WbS_stall = wind_loading_stall(V_stall, CL_max, rho)

# ╔═╡ e3ff9893-7651-412d-afb3-3c4a05ef670f
stalls = fill(WbS_stall, n);

# ╔═╡ ec1aef31-16ed-4337-95aa-eb933f3fb8ff
begin 
...
# ╔═╡ f3931580-f371-4f22-bb82-297234e821c1
# climb
begin
...
# ╔═╡ 1516a9d6-2dcb-48c3-aa1a-b918700cd1ae
# drag polar
begin
...
# ╔═╡ ed7990b1-9df6-44c6-82cf-c7526c6a1470
induced_drag_coefficient(e, AR) = 1 / (pi * e * AR)

# ╔═╡ c8f53fe9-1682-42b6-aa5f-28061933f6e4
K = induced_drag_coefficient(e, AR)

# ╔═╡ 778d8bea-e43b-49b4-adb1-006922908360
climb_condition(k_s, CD0, CL_max, K, G) = k_s^2 * CD0 / CL_max + K * CL_max / k_s^2 + G

# ╔═╡ 678400f4-496c-440f-8a86-6696ffcfe5a3
...
function thrust_corrected_climb(k_s, CD0, CL_max, K, G, n_eng, # Necessary inputs
	...
	if OEI # One-engine-out condition
...
# ╔═╡ f4af8639-60bd-4a34-a241-5f1ca51d014d
# need to define
thrust_corrected_climb(1.1, 0.001, 2.1, K, 0.1, 2, 0.94, MCT = true, OEI = true)

# ╔═╡ e5c14544-bf21-40cc-9369-d7689c82c3de
begin
...
# ╔═╡ 1dfb03bb-f0b8-414e-a4f0-6968377beb1d
...
begin	
	...
	takeoff_climb = fill(TbW_takeoff_climb, length(wing_loadings))
end

# ╔═╡ 58f66036-6988-4194-99bd-9fd38f374ee5
# transition climb OEI (Climb2)
begin	
	k_s_trans 	= 1.1
...
# ╔═╡ dfd716b4-18ca-4f70-92d9-c6945564ce78
# second climb OEI (Climb3)
begin	
	k_s_second 	= 1.2
...
# ╔═╡ f1036ac6-b0d7-440b-875a-db2e3ee76c25
# enroute climb OEI (Climb4)
begin	
	k_s_enroute  = 1.25
	G_enroute 	 = 0.012
...
# ╔═╡ 2f359d36-ceb8-4dd2-9f1b-573d9649ad40
# balked landing climb OEI (Climb5)
begin	
	k_s_baulked_OEI = 1.5
	G_baulked_OEI = 0.021
...
# ╔═╡ e5005d2d-5218-4884-9856-17952bf14605
# balked landing climb AEO (Climb6)
begin	
	k_s_baulked_AEO  = 1.3
	G_baulked_AEO    = 0.032
...
# ╔═╡ 7f503876-ea37-49e0-ad6b-3ac69d063d44
begin 
	plot(xlabel = "W/S, N/m²", ylabel = "T/W", title = "Matching Chart")
	plot!(wing_loadings, TbW_takeoff, label = "Takeoff")
	plot!(wbs_landing, TbWs, label = "Landing")
	plot!(stalls, TbWs, label = "Stall")
	plot!(wing_loadings, takeoff_climb, label = "Climb 1")
...
# ╔═╡ a8f6e606-c63e-4e75-9d23-320f9dff19a8
# cruises
begin
	σ_cruise = 0.2846
	M = 0.84 			# Mach number
	a = 302.72 			# Speed of sound at 45000 ft (-45 degree Celsius)
end

# ╔═╡ 80169df5-2137-4fbf-92aa-db00cd838c2e
q = dynamic_pressure(σ_cruise * 1.225, M * a)

# ╔═╡ 45848275-501b-4ec2-a36d-45d063978b25
k = induced_drag_coefficient(e_0deg, AR)

# ╔═╡ e26667a6-8a83-44ab-b302-92b9028a8d43
cruise_condition(wing_loading, q, CD0, k) = q * CD0 / wing_loading + k / q * wing_loading

# ╔═╡ 3d7c60bf-2840-4703-ad9f-91516b0ad16d
WF_cruise = 0.253776
# need to define and change

# ╔═╡ f92b3a24-377b-42ca-9fb4-b55a21a97325
tbw_cruise = 1/σ_cruise^0.6 * WF_cruise * cruise_condition.(wing_loadings, q, CD0, k);

# ╔═╡ 84ae8640-af83-4305-9c36-5edb62db1bf2
begin
	plot(
		xlabel = "(W/S), N/m²", 
		ylabel = "(T/W)", 
	...
	plot!(wing_loadings, TbW_takeoff, label = "Takeoff")
	plot!(wbs_landing, TbWs, label = "Landing")
	plot!(wing_loadings, takeoff_climb, label = "Takeoff Climb")
...
# ╔═╡ 6c22e7f8-34da-4ffc-9bf8-f92a262c8ac6
# ceiling 


# ╔═╡ 4458bfa9-72c2-456a-850e-367f7d68f8f3
ceiling_condition(C_D0, K, G = 0.001) = G + 2 * sqrt(C_D0 * K)

# ╔═╡ 2486151c-fc17-4148-8da1-49507d5eb173
tbw_ceiling = fill(ceiling_condition(CD0, k, 0.001), length(wing_loadings))

# ╔═╡ b8db0651-597b-4f14-8a29-d912e94290af
begin 
	plot(xlabel = "W/S, N/m²", ylabel = "T/W", title = "Matching Chart")
	plot!(wing_loadings, TbW_takeoff, label = "Takeoff")
	plot!(wbs_landing, TbWs, label = "Landing")
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "aa98c31af85e04bca73647654a55640f3d350c1d"
