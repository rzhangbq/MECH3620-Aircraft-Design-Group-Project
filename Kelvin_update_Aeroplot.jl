...
htail = WingSection(
    area        = 4.43,  			# Area (m²). HOW DO YOU DETERMINE THIS?
    aspect      = 5.0738,  			# Aspect ratio
    ...
    position    = fuse_end - [ 2.727, 0., -0.832], # HOW DO YOU DETERMINE THIS?
)
...
vtail = WingSection(
    area        = 3.34, 			# Area (m²). # HOW DO YOU DETERMINE THIS?
    aspect      = 0.844,  			# Aspect ratio
    taper       = 0.59831,  			# Taper ratio
...
begin
	...
	W_engine_fac = 1.4* W_engine # Scaling factor for engine weight
	...
	W_vtail = Wvtail(Svt, Htov, ARv, Vsweep, Lt, ttoc,Nz,Lt,Wdg)*lbtokg
	W_fuse 	= Wfuse(Sf,Nz,Wdg,Lt,qvalue,LtoD)*lbtokg*g
...
begin
	...
	r_eng_L = wing_coo[1,2] - [-2.2, 0., 0.]     # Engine, near wing LE
	r_eng_R = wing_coo[1,4] - [-2.2, 0., 0.] 	   # Engine, near wing LE
	...
	r_payload = r_nose + [0.8 * fuse.length, 0., -fuse.radius]

	######### Added Passenger location
	r_pax = r_nose + [0.4 * fuse.length, 0., -fuse.radius]

	######### Added Fuel location
	r_fuel = r_nose + [0.4 * fuse.length, 0., -fuse.radius]
