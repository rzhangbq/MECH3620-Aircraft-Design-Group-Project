...
# ╔═╡ d9e3646a-e849-486f-b037-62bc3303ca5c
using AeroFuse

# ╔═╡ 3c9a3e79-ca0d-4c1b-9cb5-174c8b18f0bb
using Plots

# ╔═╡ d625dc2e-dc13-4bcb-95e1-d2d4609c54c1
using DataFrames

# ╔═╡ 213450e2-1c71-44a6-bbab-ea021b56061b
### A Pluto.jl notebook ###
# v0.19.38
begin
	using Markdown
...
# ╔═╡ d589a931-8160-4b3b-83a2-04b7fa1d3319
gr(
...
begin
	""" These are variables """
	g = 9.81
	Wpayload = 1000*g #in N
	Wcrew = 200*g 	  #in N
	Cruise_speed = 810*1000/3600 #in m/s
	LD_cruise = 11.5
	R1 = 4500*1000 #in meter
...
loiterFF = loiter_weight_fraction(45*60,cruise_SFC*1.3,LD_cruise)
...
# ╔═╡ 39c33d11-7439-42ce-aadd-e5a8e4c269f4
...
# ╔═╡ 273a6349-7a21-4f26-821c-12d41a3b38c9
...
# ╔═╡ 46b17ac8-0bc3-4e09-9da5-a5c44a87febd
stalls = fill(WbS_stall, n);

# ╔═╡ b3cf58af-410d-45c3-bf2c-4b1f6afb45b5
begin 
...
# ╔═╡ 8d26036e-ddea-4e80-a45a-a25ddfc76171
# drag polar
begin
	e = 0.75 	# efficiency factor
	AR = 7.8671328671328675 	# aspect ratio
end
...
begin
	...
	CD0 = CL_required/12 - CL_required^2/(pi * e * AR)
end

# ╔═╡ 8531bd47-a6ce-4c5d-bd1e-c5ccd03a7122
induced_drag_coefficient(e, AR) = 1 / (pi * e * AR)

# ╔═╡ a409f8c0-8a05-4dd6-8cab-460fd1eb2781
K = induced_drag_coefficient(e, AR)

# ╔═╡ 19527fbb-6a82-45a9-b5d7-ca650f5e3c99
...
# ╔═╡ df61ce33-ae50-456f-9147-8bfb745feb8d
...
# ╔═╡ 41316675-9c9f-44bd-a9ca-e15d38372c45
...
# ╔═╡ dc36aa2c-bf3e-4492-8cf4-78ae0bac1f45
...
# ╔═╡ 9c7c37bc-1e87-4f60-884b-23df8542c666
begin 
	plot(xlabel = "W/S, N/m²", ylabel = "T/W", title = "Matching Chart")
	plot!(wing_loadings, TbW_takeoff, label = "Takeoff")
	plot!(wbs_landing, TbWs, label = "Landing")
...
# ╔═╡ 3c1f86e5-4182-4af1-a5d3-3c9c47f82a57
# cruises
begin
	σ_cruise = 0.2846
	M = 0.84 			# Mach number
...
# ╔═╡ 6c23be2d-eb22-4d1f-b78f-57f88e490450
q = dynamic_pressure(σ_cruise * 1.225, M * a)

# ╔═╡ 96321d26-45a2-4fe4-a0e4-933e59a23f6d
k = induced_drag_coefficient(e_0deg, AR)

# ╔═╡ f0d2c59d-c676-4d6c-8d24-3920b8ecf95b
...
# ╔═╡ 1acf4cfe-7aee-4908-88ca-1234a31fa8b6
#=╠═╡
begin
	plot(
		xlabel = "(W/S), N/m²", 
...
# ╔═╡ abcb0572-c125-4658-8fd2-80a0c7a29ea9
begin
	CJ3weight = 6291*g
	CJ3wing = 27.32
	CJ3Thrust = 22.2*10^3
	...
	Phenoweight = 8414.59*g
	Phenowingloading = 286*g
	PhenoThrust = 15.471*2*10^3
	
	Hawkerweight = 7393.5*g
	HawkerThrust = 28*10^3
	Hawkerwing = 22.4

	
	Learjetweight = 9800*g
	LearjetTtoW = 0.438
	Learjetwing = 28.95
	
	Citationxweight=  9200*g
	CitationxThrust = 18.32*2*10^3
	Citationxwing = 34.35
	
	Citationlweight = 13971*g
	Citationlwing = 50.4
	CitationlThrust = 26275*2
...
begin 
	...
	scatter!([last(WTOs)/27], [32000*0.8/last(WTOs)], label = "Ours")
	scatter!([CJ3weight/CJ3wing], [CJ3Thrust/CJ3weight], label = "CJ3")
	scatter!([CJ4weight/CJ4wing], [CJ4Thrust/CJ4weight], label = "CJ4")
	scatter!([Phenowingloading], [PhenoThrust/Phenoweight], label = "Phenom 300")
	scatter!([Hawkerweight/Hawkerwing], [HawkerThrust/Hawkerweight], label = "Hawker400 XPR")
	scatter!([Learjetweight/Learjetwing], [LearjetTtoW], label = "Learjet")
	scatter!([Citationxweight/Citationxwing], [CitationxThrust/Citationxweight], label = "Citation XLS")
	scatter!([Citationlweight/Citationlwing], [CitationlThrust/Citationlweight], label = "Citation latitude")
	scatter!([PCweight/PCwing], [PCThrust/PCweight], label = "PC-24")
	
end
  ╠═╡ =#

# ╔═╡ 7db95481-787f-4d43-ac34-ee803f31baf2


# ╔═╡ 28f32b3f-aecf-4822-943e-e8e6be19583f
...
#=╠═╡
[last(WTOs)/26.6], [32000/last(WTOs)]
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "e835307c71f2babecb8d429263a64498e2f2c69f"
