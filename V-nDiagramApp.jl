...
# ╔═╡ 190a84c9-f0fd-4170-a934-f34f825477d2
...
# ╔═╡ 7a08df17-2588-4a6e-b2ac-d6de7b97f7e8
md"### Cost Estimation"

# ╔═╡ b7f4a51c-08f2-4bf4-8853-1a9bfb67deed


begin
	#### Fuel and airport maintenance
	Cfuel(Wf,pf,rhof) = 1.02*Wf*pf/rhof
	Cairport(MTOW,CEF) = 4.25*(MTOW)*CEF
	Cnavigation(CEF,R,tb,MTOW) = 0.5*CEF*(1.852*R/tb)*sqrt(0.00045359237*MTOW/50)
	Caircraft(MTOW,CEF) = 10^(3.3191+0.8043*log(10,MTOW))*CEF



	
	Cserv(npax, tb, CEF) = (0.285+0.0025)*(npex/tb)*CEF
	Cfood(npax, Rp, CEF) = 1.05*(2.42*(npax)/(1+Rp)+npax*Rp/(1+Rp))*CEF
	Cent(tb, CEF) = 196/tb*CEF
	Cpaxins(npax, PLF, R, tb, CEF) = 0.52*(npax*PLF*R/(1000*tb))*CEF
	Cpayloadhandling(npax, PLF, Wcargo, CEF) = 2.87*(npax*PLF/tb)*CEF+1.31*(npax*PLF/tb)*CEF + 131.08*(Wcargo/(2000*tb))*CEF
	Cres(npax, PLF, tb, CEF) = 4.4*(npax*PLF/tb)*CEF
	Cpub(R,Cf,npax,PLF,tb) = 0.023*(R*Cf*npax/tb)*CEF
	Cmarketing(Cres,Cpub) = Cres+Cpub
	Ccommon(npax,PLF,R,tb,CEF) = 2.35*(npax*PLF*R/(1000*tb))*CEF
	Cgen(npax,tb) = (0.18*npax/tb)*CEF
	Cadmin(Cgen,Ccomm) = Cgen+Ccomm
end
...
begin
	# Aircraft quantities
	S_ref 	  	= 27	    # Reference area, m²
	chord 	  	= 2.1293 		# Reference chord length, m
	CL_alpha  	= 0.088 * 180 / pi		# Lift curve slope, /rad (USE DATCOM OR AEROFUSE)
	CL_max 	  	=  1.4		# Maximum CL (flaps retracted)
	CL_min 	  	= -CL_max/2		# Minimum CL (flaps retracted)

	# Weight quantites
	max_WS 		= 2700 				# Maximum wing loading, N/m²
	factor 		= 0.94   		   	# % of the max wing loading (CRUISE WEIGHT!)
	WbyS 		= factor * max_WS  	# Wing loading

	# Altitude and density relations
	unit    	= "ft" 			   			 # Altitude units ("ft" or "m")
	alt 		= 45000. 		   			 # Cruise altitude
	V_cruise  	= 810/3.6 					# Cruise speed, m/s
...
# ╔═╡ 534cdc4d-2d41-4be4-b520-ea4552685833
sigma
...
function n_gust(Kg, CL_alpha, Ue, V, WbS; positive = true)
	1 + ifelse(positive, 1, -1) * Kg * CL_alpha * Ue * V / 2WbS
end
...
begin
	...
	basic = plot(
	    ...
	    title  = "V-n Diagram, Basic Flight Envelope, Cruise",
	    legend = :bottomleft,
...
begin
	...
	gust = plot(
	    xlabel = "Equivalent Airspeed, V_EAS (m/s)",
	    ylabel = "Load Factor, n",
	    title  = "V-n Diagram, Gust Envelope, Cruise",
	    legend = :bottomleft,
